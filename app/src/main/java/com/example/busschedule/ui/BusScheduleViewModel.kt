...
import com.example.busschedule.ApplicationBusSchedule
...
class BusScheduleViewModel(private val busScheduleDao: BusScheduleDao) : ViewModel() {
    ...
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> =
        busScheduleDao.getBusSchedule(stopName)


    companion object {
        val factory: ViewModelProvider.Factory = viewModelFactory {
            initializer {
                val app = (this[APPLICATION_KEY] as ApplicationBusSchedule)
                BusScheduleViewModel(app.dataBase.busSchedule())
            }
        }
    }
}
